...
class rigidBody(body):
    ...
    def animate(self,COGPos,rotation):
        for actor in self.actors:
            ...
            diffRot = np.linalg.matmul(iniPAI.T,iniRot)                   #siehe Mitschrift
            posLocal = np.linalg.matmul(iniPAI.T,np.array(self.parameter["position"]["value"]) - np.array(self.parameter["COG"]["value"]))   # posLocal = R
